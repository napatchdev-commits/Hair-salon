...
import { UserCheck, Plus, Edit2, Calendar, Clock, Trash2, Loader2, CheckCircle2, XCircle } from 'lucide-react';
  ...
  async function handleSaveStaff() {
    ...
    try {
      if (editingStaff) {
        ...
        // Insert staff and default weekly schedule (Mon-Sun 10:00-19:00)
      ...
      alert(err.message || 'ไม่สามารถบันทึกข้อมูลช่างได้');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-black text-white">จัดการข้อมูลช่างทำผม (Staff)</h1>
          <p className="text-xs text-slate-400">เพิ่ม/แก้ไขช่าง กำหนดวันทำงาน เวลาพัก และวันหยุดช่าง</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-slate-400">
          ...
          <span className="ml-2 text-xs">กำลังโหลดข้อมูลช่าง...</span>
        </div>
      ) : staffList.length === 0 ? (
        <div className="bg-slate-800 border border-slate-700/60 rounded-2xl p-12 text-center text-xs text-slate-500">
          ยังไม่มีข้อมูลช่างทำผม กรุณากดปุ่ม "เพิ่มช่างทำผมใหม่" ด้านบน
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {staffList.map((st) => (
            <div
              key={st.id}
              className="bg-slate-800 border border-slate-700/60 p-5 rounded-2xl space-y-4 shadow-sm"
            >
              <div className="flex justify-between items-start">
                <div className="flex items-center space-x-3">
                  <div className="w-12 h-12 rounded-full bg-salon-700/40 text-salon-300 font-black text-xl flex items-center justify-center border border-salon-500/30">
                    {st.name.charAt(0)}
                  </div>
                  <div>
              ...
              <div className="flex space-x-2 pt-2 border-t border-slate-700/50 text-xs font-semibold">
                <button
                  onClick={() => openEditModal(st)}
                  className="flex-1 py-2 bg-slate-900 hover:bg-slate-700 text-slate-200 rounded-xl flex items-center justify-center space-x-1"
                >
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-salon-700/30 hover:bg-salon-700/50 text-salon-300 rounded-xl flex items-center justify-center space-x-1 border border-salon-500/30"
                >
      ...
      {showStaffModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-3xl p-6 w-full max-w-md space-y-4 shadow-2xl">
            <h3 className="font-bold text-sm text-white">
              {editingStaff ? 'แก้ไขข้อมูลช่างทำผม' : 'เพิ่มช่างทำผมใหม่'}
            </h3>

            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">ชื่อช่าง <span className="text-rose-400">*</span></label>
                <input
                  type="text"
      ...
      {showScheduleModal && activeTabStaff && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-3xl p-6 w-full max-w-2xl space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-700 pb-3">
              <div>
                <h3 className="font-bold text-base text-white">
                  ตารางทำงานและวันหยุด: ช่าง{activeTabStaff.name}
                </h3>
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-salon-400 uppercase tracking-wider">
                ตารางทำงานประจำสัปดาห์ (Working Days)
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                {schedules.map((sc) => (
                  <div key={sc.id} className="bg-slate-900 p-3 rounded-xl border border-slate-700 flex items-center justify-between">
                    <div>
            ...
            <div className="space-y-3 pt-2 border-t border-slate-700/60">
              ...
              <div className="bg-slate-900 p-3 rounded-xl border border-slate-700 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                <input
                  type="date"
                  value={holidayDate}
                  onChange={(e) => setHolidayDate(e.target.value)}
                  className="p-2 bg-slate-800 border border-slate-700 rounded-lg text-white font-semibold outline-none"
                />
                <input
                  type="text"
                  ...
                  className="p-2 bg-slate-800 border border-slate-700 rounded-lg text-white outline-none"
                />
                <button
                  onClick={handleAddHoliday}
                  className="py-2 bg-salon-600 hover:bg-salon-500 text-white font-bold rounded-lg shadow-md"
                >
                  + เพิ่มวันหยุด
                </button>
              </div>
              ...
              {holidays.length > 0 && (
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {holidays.map((h) => (
                    <div key={h.id} className="bg-slate-900 p-2.5 rounded-xl border border-slate-700 flex justify-between items-center text-xs">
                      <div>

...
import { Calendar, Clock, DollarSign, Users, XCircle, AlertTriangle, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        ...
        <p className="mt-2 text-xs">กำลังโหลดข้อมูลสรุปแดชบอร์ด...</p>
      </div>
    );
  }

  const statCards = [
    { label: 'คิววันนี้', value: stats.todayCount, icon: Calendar, color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
    { label: 'คิวพรุ่งนี้', value: stats.tomorrowCount, icon: Clock, color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
    { label: 'จำนวนคิวทั้งหมด', value: stats.totalCount, icon: CheckCircle2, color: 'text-purple-400 bg-purple-500/10 border-purple-500/20' },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-black text-white">ภาพรวมระบบ (Dashboard Metrics)</h1>
      ...
      <div className="bg-slate-800 border border-slate-700/60 rounded-2xl p-5 space-y-4">
        <h2 className="text-sm font-bold text-white flex items-center">
          <Clock className="w-4 h-4 text-salon-400 mr-2" />
          รายการจองคิวล่าสุด (Recent Bookings)
        </h2>

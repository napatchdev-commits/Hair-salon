...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Filter, UserCheck, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-black text-white">ปฏิทินตารางคิว (Calendar View)</h1>
          <p className="text-xs text-slate-400">แสดงผังตารางเวลานัดหมายแบบรายวันและรายสัปดาห์</p>
        </div>

        {/* View mode toggle */}
        <div className="flex items-center space-x-2 bg-slate-800 p-1 rounded-xl border border-slate-700/60 text-xs">
          <button
            onClick={() => setViewMode('day')}
            className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
              viewMode === 'day' ? 'bg-salon-700 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            รายวัน (Day)
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
              viewMode === 'week' ? 'bg-salon-700 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            รายสัปดาห์ (Week)
          </button>
      ...
      <div className="bg-slate-800 border border-slate-700/60 p-4 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-3 text-xs">
        <div className="flex items-center space-x-3">
          <button
            onClick={() => setSelectedDate(subDays(selectedDate, viewMode === 'day' ? 1 : 7))}
            className="p-2 bg-slate-900 border border-slate-700 text-slate-300 hover:text-white rounded-xl"
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-slate-700 text-slate-200 font-bold rounded-xl hover:bg-slate-600"
          >
            วันนี้
          </button>
        </div>
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <UserCheck className="w-4 h-4 text-salon-400" />
          <select
            value={selectedStaffId}
            onChange={(e) => setSelectedStaffId(e.target.value)}
            className="p-2 bg-slate-900 border border-slate-700 rounded-xl text-white outline-none font-medium text-xs w-full sm:w-48"
          >
      ...
      {loading ? (
        ...
        <div className="bg-slate-800 border border-slate-700/60 rounded-2xl p-5 space-y-4">
          <h2 className="text-sm font-bold text-white flex items-center">
            <CalendarIcon className="w-4 h-4 text-salon-400 mr-2" />
            ตารางคิวประจำวันที่ {formatThaiDate(currentDateStr)}
          </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {appointments.map((appt) => {
                ...
                return (
                  <div
                    key={appt.id}
                    className="bg-slate-900/90 border border-slate-700 p-4 rounded-xl space-y-2 text-xs"
                  >
                    <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                      <span className="font-bold text-salon-400 text-sm">#{appt.queue_number}</span>
                    ...
                    <div className="space-y-1 text-slate-300">
                      ...
                      <p><span className="text-slate-500">บริการ:</span> <strong className="text-slate-200">{appt.services?.name}</strong></p>
                      <p><span className="text-slate-500">ช่าง:</span> <strong className="text-slate-200">ช่าง{appt.staff?.name}</strong></p>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
          {weekDays.map((dayDate) => {
            ...
            return (
              <div
                key={dayStr}
                className={`bg-slate-800 border rounded-2xl p-3 flex flex-col min-h-64 space-y-2 ${
                  isToday ? 'border-salon-500 ring-1 ring-salon-500/50' : 'border-slate-700/60'
                }`}
              >
                <div className="border-b border-slate-700 pb-2 text-center">
                  ...
                  <p className={`text-sm font-extrabold ${isToday ? 'text-salon-400' : 'text-white'}`}>
                ...
                <div className="flex-1 space-y-2 overflow-y-auto max-h-96">
                  {dayAppts.length === 0 ? (
                    ...
                    dayAppts.map((appt) => (
                      <div
                        key={appt.id}
                        className="bg-slate-900 border border-slate-700/80 p-2 rounded-lg text-[11px] space-y-0.5"
                      >
                        <div className="font-bold text-salon-400">{formatTimeSlot(appt.start_time)}</div>

...
import { Clock, Search, Filter, Edit3, XCircle, CheckCircle2, Loader2, RefreshCw } from 'lucide-react';
...
export default function AdminAppointmentsPage() {
  ...
  async function handleUpdateStatus() {
    ...
    try {
      ...
      if (error) throw error;
      alert('อัปเดตสถานะคิวเรียบร้อยแล้ว');
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-black text-white">จัดการรายการคิว (Appointments)</h1>
          <p className="text-xs text-slate-400">ค้นหา กรองข้อมูล เปลี่ยนสถานะ หรือแก้ไขเวลาจองคิว</p>
        </div>
        <button
      ...
      {/* Filters Bar */}
      <div className="bg-slate-800 border border-slate-700/60 p-4 rounded-2xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
        {/* Search */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-white outline-none focus:border-salon-500"
          />
        </div>
        ...
        <select
          ...
          className="w-full p-2 bg-slate-900 border border-slate-700 rounded-xl text-white outline-none focus:border-salon-500 font-medium"
        >
      ...
      <div className="bg-slate-800 border border-slate-700/60 rounded-2xl overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-20 text-slate-400">
            ...
            <span className="ml-2 text-xs">กำลังค้นหารายการคิว...</span>
          </div>
            ...
            <table className="w-full text-left text-xs text-slate-300">
              ...
              <tbody className="divide-y divide-slate-700/40">
                {appointments.map((appt) => {
                  ...
                  return (
                    <tr key={appt.id} className="hover:bg-slate-700/20">
                      <td className="p-3 font-bold text-salon-400">#{appt.queue_number}</td>
                      <td className="p-3">
                      ...
                      <td className="p-3">ช่าง{appt.staff?.name}</td>
                      <td className="p-3">
                        <div>{formatThaiDate(appt.booking_date)}</div>
                        <div className="text-salon-400 font-bold">{formatTimeSlot(appt.start_time)}</div>
                      </td>
                      ...
                      <td className="p-3 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 bg-salon-700 hover:bg-salon-600 text-white font-semibold rounded-lg text-[11px]"
                        >
                          เปลี่ยนสถานะ
                        </button>
                      </td>
      ...
      {selectedAppt && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 border border-slate-700 rounded-3xl p-6 w-full max-w-sm space-y-4 shadow-2xl">
            ...
            <div className="text-xs text-slate-400 space-y-1">
              ...
              <p>บริการ: <span className="text-white font-semibold">{selectedAppt.services?.name}</span></p>
            </div>
            ...
            <div className="flex space-x-2 pt-2 text-xs font-bold">
              <button
                ...
                className="flex-1 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-xl"
              >
                ยกเลิก
              </button>
              <button
                ...
                className="flex-1 py-2.5 bg-salon-600 hover:bg-salon-500 text-white rounded-xl shadow-md"
              >

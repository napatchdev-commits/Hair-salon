...
import { Calendar, Clock, Scissors, UserCheck, MapPin, Sparkles } from 'lucide-react';
...
export default function LiffLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: '/liff/services', label: 'บริการ', icon: Scissors },
    { href: '/liff/staff', label: 'ช่าง', icon: UserCheck },
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between max-w-md mx-auto shadow-2xl relative border-x border-slate-200 font-sans">
      ...
      <header className="bg-gradient-to-r from-salon-800 via-salon-700 to-salon-900 text-white px-5 py-4 flex items-center justify-between sticky top-0 z-30 shadow-md">
        <div className="flex items-center space-x-2">
          <div className="bg-white/10 p-2 rounded-xl backdrop-blur-sm border border-white/20">
            <Sparkles className="w-5 h-5 text-salon-gold" />
          </div>
          <div>
            <h1 className="font-bold text-lg tracking-wide text-white">HAIR SALON</h1>
            <p className="text-[10px] text-salon-200">LINE Booking & Queue System</p>
          </div>
        </div>
      </header>
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center py-1 px-2 rounded-xl transition-all duration-200 ${
                isActive
                  ? 'text-salon-700 font-bold scale-105'
                  : 'text-slate-500 hover:text-slate-700 font-medium'
              }`}
            >
              <div className={`p-1.5 rounded-xl ${isActive ? 'bg-salon-100' : 'bg-transparent'}`}>
                <Icon className={`w-5 h-5 ${isActive ? 'text-salon-700' : 'text-slate-500'}`} />
              </div>

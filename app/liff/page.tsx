...
import { Scissors, UserCheck, Calendar as CalendarIcon, Clock, CheckCircle2, AlertCircle, ChevronRight, ArrowLeft, Loader2, Sparkles } from 'lucide-react';
  ...
  if (loading && step === 1 && services.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <Loader2 className="w-8 h-8 text-salon-600 animate-spin" />
        <p className="mt-3 text-sm text-slate-500">กำลังโหลดข้อมูลบริการ...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {step <= 5 && (
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-sm text-salon-900">
              {step === 1 && 'ขั้นตอนที่ 1/5: เลือกบริการ'}
              {step === 2 && 'ขั้นตอนที่ 2/5: เลือกช่างทำผม'}
              {step === 3 && 'ขั้นตอนที่ 3/5: เลือกวันที่'}
              {step === 4 && 'ขั้นตอนที่ 4/5: เลือกเวลา'}
              {step === 5 && 'ขั้นตอนที่ 5/5: ยืนยันข้อมูล'}
            </span>
          </div>
          <span className="text-xs font-semibold text-salon-600 bg-salon-50 px-2.5 py-1 rounded-full">
            {step} / 5
          </span>
        </div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-3">
          <h2 className="text-base font-bold text-slate-800 flex items-center">
            <Scissors className="w-5 h-5 text-salon-600 mr-2" />
                ...
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center justify-between shadow-sm hover:shadow-md ${
                    selectedService?.id === srv.id
                      ? 'border-salon-600 bg-salon-50/50 ring-2 ring-salon-500/20'
                      : 'border-slate-200 bg-white hover:border-salon-300'
                  }`}
                >
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center space-x-3 pt-1 text-xs text-slate-600">
                      <span className="font-semibold text-salon-700">{formatCurrency(srv.price)}</span>
                      <span>•</span>
              ...
              {staffList.map((st) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className="w-12 h-12 rounded-full bg-salon-100 text-salon-700 flex items-center justify-center font-bold text-lg border border-salon-200">
                      {st.name.charAt(0)}
                    </div>
                    <div>
      ...
      {step === 3 && (
        <div className="space-y-3">
          <h2 className="text-base font-bold text-slate-800 flex items-center">
            <CalendarIcon className="w-5 h-5 text-salon-600 mr-2" />
            เลือกวันที่จอง
          </h2>

          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
            <label className="block text-xs font-semibold text-slate-700">วันที่ต้องการรับบริการ:</label>
            <input
              type="date"
              value={bookingDate}
              min={new Date().toISOString().split('T')[0]}
              onChange={(e) => setBookingDate(e.target.value)}
              className="w-full p-3 border border-slate-300 rounded-xl text-sm font-semibold focus:ring-2 focus:ring-salon-500 outline-none"
            />
            <p className="text-[11px] text-slate-500">
              วันที่เลือก: <span className="font-bold text-salon-700">{formatThaiDate(bookingDate)}</span>
            </p>
          </div>
          ...
          <button
            ...
            onClick={() => setStep(4)}
            className="w-full py-3.5 bg-salon-700 hover:bg-salon-800 text-white font-bold rounded-xl text-sm shadow-md transition-all flex items-center justify-center space-x-2"
          >
      ...
      {step === 4 && (
        <div className="space-y-3">
          <h2 className="text-base font-bold text-slate-800 flex items-center">
            <Clock className="w-5 h-5 text-salon-600 mr-2" />
            เวลาที่เปิดให้จอง
          </h2>

          <div className="bg-salon-50/70 border border-salon-200/60 p-3 rounded-xl text-xs space-y-1 text-salon-900">
            <p>✂️ Service: <span className="font-bold">{selectedService?.name}</span> ({selectedService?.duration_minutes} นาที)</p>
            <p>👩‍🦰 Staff: <span className="font-bold">ช่าง{selectedStaff?.name}</span></p>
            <p>📅 Date: <span className="font-bold">{formatThaiDate(bookingDate)}</span></p>
          </div>
          ...
          {loadingSlots ? (
            ...
            <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl text-center space-y-2">
              <p className="text-xs text-amber-800 font-semibold">
                ไม่มีรอบเวลาว่างสำหรับช่างและวันที่เลือก
              </p>
              <p className="text-[11px] text-amber-700">
                (ช่างอาจจะหยุดงาน ตรงกับเวลาพัก หรือมีคิวจองเต็มแล้ว)
              </p>
              <button
                onClick={() => setStep(3)}
                className="mt-2 text-xs text-salon-700 underline font-bold"
              >
                เปลี่ยนวันที่จอง
              </button>
            </div>
                ...
                <button
                  ...
                  className={`py-3 px-2 rounded-xl text-xs font-bold transition-all border ${
                    selectedTime === slotTime
                      ? 'bg-salon-700 text-white border-salon-800 shadow-md ring-2 ring-salon-400/40'
                      : 'bg-white text-slate-700 border-slate-200 hover:border-salon-300'
                  }`}
                >
          ...
          <button
            ...
            className={`w-full py-3.5 font-bold rounded-xl text-sm shadow-md transition-all flex items-center justify-center space-x-2 ${
              selectedTime
                ? 'bg-salon-700 hover:bg-salon-800 text-white'
      ...
      {step === 5 && (
        <div className="space-y-3">
          <h2 className="text-base font-bold text-slate-800 flex items-center">
            <CheckCircle2 className="w-5 h-5 text-salon-600 mr-2" />
            ข้อมูลผู้จอง & สรุปคิว
          </h2>

          {/* Booking Summary Card */}
          <div className="bg-gradient-to-br from-salon-900 to-salon-800 text-white p-4 rounded-2xl shadow-md space-y-2 text-xs">
            <div className="flex justify-between items-center border-b border-white/10 pb-2">
              <span className="text-salon-200 font-medium">บริการ</span>
              <span className="font-bold text-sm text-white">{selectedService?.name}</span>
            </div>
            <div className="flex justify-between items-center border-b border-white/10 pb-2">
              <span className="text-salon-200 font-medium">ช่างทำผม</span>
              <span className="font-bold text-white">ช่าง{selectedStaff?.name}</span>
            </div>
            <div className="flex justify-between items-center border-b border-white/10 pb-2">
              <span className="text-salon-200 font-medium">วันที่ & เวลา</span>
              <span className="font-bold text-salon-gold">{formatThaiDate(bookingDate)} @ {formatTimeSlot(selectedTime)}</span>
            </div>
            <div className="flex justify-between items-center pt-1">
              <span className="text-salon-200 font-medium">ยอดรวมทั้งสิ้น</span>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 border border-slate-300 rounded-xl outline-none focus:ring-2 focus:ring-salon-500 font-medium"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                placeholder="เช่น ทรงผมที่ต้องการ หรือการแพ้สารเคมี"
                rows={2}
                value={note}
      ...
      {step === 6 && bookingResult && (
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xl text-center space-y-4 my-4">
          ...
          <div className="space-y-1">
            <h2 className="text-xl font-black text-slate-900">จองคิวสำเร็จแล้ว!</h2>
            <p className="text-xs text-slate-500">ระบบได้บันทึกคิวและแจ้งเตือนผ่าน LINE เรียบร้อยแล้ว</p>
          </div>

          <div className="bg-salon-50 border border-salon-200 p-4 rounded-2xl text-salon-900 space-y-1">
            <p className="text-xs text-slate-500 uppercase tracking-widest font-semibold">หมายเลขคิวของคุณ</p>
            <p className="text-2xl font-black text-salon-700 tracking-wider">{bookingResult.queueNumber}</p>
          </div>
          ...
          <div className="text-left text-xs space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <p><span className="text-slate-500">ช่าง:</span> <strong className="text-slate-900">ช่าง{selectedStaff?.name}</strong></p>
          ...
          <a
            href="/liff/my-queue"
            className="block w-full py-3 bg-salon-700 text-white font-bold rounded-xl text-xs shadow-md"
          >
            ดูรายการคิวของฉัน
          </a>
        </div>

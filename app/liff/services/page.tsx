  ...
  return (
    <div className="space-y-4">
      <h2 className="text-base font-bold text-slate-800 flex items-center">
        ...
        บริการและอัตราค่าบริการ (Services)
      </h2>

      {loading ? (
        <div className="flex flex-col items-center justify-center py-16">
          <Loader2 className="w-6 h-6 text-salon-600 animate-spin" />
          <p className="mt-2 text-xs text-slate-500">กำลังโหลดรายการบริการ...</p>
        </div>
      ) : services.length === 0 ? (
          ...
          {services.map((srv) => (
            <div key={srv.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-2">
              <div className="flex justify-between items-start">
                ...
                <span className="font-extrabold text-sm text-salon-700 bg-salon-50 px-2.5 py-1 rounded-lg border border-salon-200">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                ...
                <Link
                  href="/liff"
                  className="text-xs font-bold text-salon-700 hover:text-salon-800 underline"
                >
                  จองบริการนี้
                </Link>
              </div>

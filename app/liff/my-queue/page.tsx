...
import { Clock, Calendar, Scissors, UserCheck, XCircle, RefreshCw, AlertCircle, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="p-2 text-slate-500 hover:text-salon-700 rounded-full"
        >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          <Loader2 className="w-7 h-7 text-salon-600 animate-spin" />
          <p className="mt-2 text-xs text-slate-500">กำลังดึงข้อมูลคิว...</p>
        </div>
      ) : appointments.length === 0 ? (
        <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-3 shadow-sm">
          <div className="w-12 h-12 bg-salon-50 text-salon-600 rounded-full flex items-center justify-center mx-auto">
            <Calendar className="w-6 h-6" />
          </div>
          <h3 className="font-bold text-sm text-slate-800">ยังไม่มีรายการคิว</h3>
          <p className="text-xs text-slate-500">คุณยังไม่ได้ทำการจองคิวทำผม สามารถกดจองได้เลย</p>
          <a
            href="/liff"
            className="inline-block px-5 py-2.5 bg-salon-700 text-white text-xs font-bold rounded-xl shadow-md"
          >
            + จองคิวตอนนี้
          </a>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between items-center border-b border-slate-100 pb-2.5">
                  <span className="font-black text-sm text-salon-900 tracking-wider">
                    #{appt.queue_number}
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center space-x-1.5 text-slate-700">
                    <Scissors className="w-3.5 h-3.5 text-salon-600 shrink-0" />
                    <span className="font-semibold line-clamp-1">{appt.services?.name}</span>
                  </div>
                  <div className="flex items-center space-x-1.5 text-slate-700">
                    <UserCheck className="w-3.5 h-3.5 text-salon-600 shrink-0" />
                    <span>ช่าง{appt.staff?.name}</span>
                  </div>
                  <div className="flex items-center space-x-1.5 text-slate-700">
                    <Calendar className="w-3.5 h-3.5 text-salon-600 shrink-0" />
                    <span>{formatThaiDate(appt.booking_date)}</span>
                  </div>
                  <div className="flex items-center space-x-1.5 text-slate-700">
                    <Clock className="w-3.5 h-3.5 text-salon-600 shrink-0" />
                    <span className="font-bold text-salon-700">{formatTimeSlot(appt.start_time)}</span>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-5 w-full max-w-sm space-y-4 shadow-2xl">
            <h3 className="font-bold text-sm text-slate-900 flex items-center">
              <RefreshCw className="w-4 h-4 text-salon-600 mr-2" />
              เลื่อนนัดหมาย (#{selectedAppt.queue_number})
            </h3>
                    ...
                    {availableSlots.map((st) => (
                      <button
                        ...
                        className={`py-2 rounded-lg text-xs font-bold border ${
                          newTime === st
                            ? 'bg-salon-700 text-white border-salon-800'
            ...
            <div className="flex space-x-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-salon-700 text-white font-bold rounded-xl text-xs shadow-md disabled:bg-slate-300"
              >
